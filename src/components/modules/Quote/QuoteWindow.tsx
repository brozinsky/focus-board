...
const QuoteWindow = () => {
  ...
  useEffect(() => {
    if (lastRefreshTime) {
      const remainingTime = 300000 - (Date.now() - lastRefreshTime);
  ...
  return (
    <blockquote className="group relative m-4 glass-blur bg-black/5 rounded-lg p-8 pr-12 max-w-[400px]">
      ...
      <ButtonIcon
        className="group-hover:opacity-100 opacity-0 z-20 -top-2 -left-2 absolute"
      ...
      {!isDisabled && (
        <button
          ...
          title="New quote"
          disabled={isDisabled}
          className={`group-hover:opacity-100 opacity-0 cursor-pointer right-2 bottom-2 absolute transition p-2 roundel-lg rounded-md 
          ${
            isDisabled
      ...
      {/* <svg
        className="absolute top-0 start-0 size-16 text-gray-100 opacity-20"
        width="16"
        height="16"
        viewBox="0 0 16 16"
        fill="none"
      ...
      <ScrollArea className="h-[5.25rem] pr-3">
      ...
      {data.author && (
        <div className="text-base font-light text-text-white/80">
          {data.author}
        </div>
      )}
    </blockquote>
