  ...
  return (
    <blockquote className="group relative m-4 glass-blur bg-black/5 rounded-lg p-8 pr-12 max-w-[400px]">
      <button
        onClick={() => refetch()}
        title="New quote"
        className="group-hover:opacity-100 opacity-0 cursor-pointer right-2 bottom-2 absolute hover:bg-white/20 transition p-2 roundel-lg hover:opacity-100 rounded-md"
      >
        <RefreshSVG width="20" />
      </button>
      <svg
      ...
      <div className="text-base font-light text-text-white/80">{data.cite}</div>
    </blockquote>
  );
};
